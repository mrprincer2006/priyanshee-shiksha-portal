...
const Dashboard = ({ onLogout }: DashboardProps) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (studentsResult.data) {
        const mappedStudents: Student[] = studentsResult.data.map(s => ({
          id: s.id,
          name: s.name,
          class: s.class,
      ...
      if (feesResult.data) {
        const mappedFees: FeeRecord[] = feesResult.data.map(f => ({
          id: f.id,
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      const { data } = supabase.storage.from('student-profiles').getPublicUrl(fileName);
  ...
  const stats = useMemo(() => {
    ...
    const feesCollected = currentMonthFees.filter(f => f.status === 'paid').reduce((sum, f) => sum + f.amount, 0);
    const pendingFees = currentMonthFees.filter(f => f.status === 'unpaid').reduce((sum, f) => sum + f.amount, 0);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <StatCard title={t('totalStudents')} value={stats.totalStudents} icon={Users} variant="primary" />
          <StatCard title={t('feesCollected')} value={`₹${stats.feesCollected.toLocaleString()}`} subtitle={t('thisMonth')} icon={IndianRupee} variant="success" />
          <StatCard title={t('pendingFees')} value={`₹${stats.pendingFees.toLocaleString()}`} subtitle={t('thisMonth')} icon={AlertCircle} variant="destructive" />
        </div>
        ...
        <div className="flex items-center justify-between my-6">
          <h2 className="text-lg font-semibold text-foreground">{t('students')} ({filteredStudents.length})</h2>
